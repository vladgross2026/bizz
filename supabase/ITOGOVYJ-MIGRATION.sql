-- =============================================================================
-- ИТОГОВАЯ МИГРАЦИЯ — всё в одном файле
-- Выполните в Supabase: SQL Editor → New query → вставьте весь файл → Run
-- Повторный запуск безопасен (IF NOT EXISTS, IF EXISTS и т.п.)
-- =============================================================================
...
-- 2. Уведомления (нужны ДО триггера admin-chat)
...
-- 3. Чаты: conversations, messages, participants
...
-- 5. Категория «Полезное» (для платных постов)
INSERT INTO categories (id, name, slug) VALUES ('useful', 'Полезное', 'useful') ON CONFLICT (id) DO NOTHING;

-- 5. Админ-чат: create_or_get_admin_conversation, get_admin_conversations_list, admin_send_message, admin_get_messages
...
-- 6. Триггер: админ получает уведомление когда пользователь пишет в админ-чат
...
BEGIN
  ...
  IF v_sender_is_admin THEN RETURN NEW; END IF;
  FOR r IN SELECT id FROM profiles WHERE secret_word = 'admingrosskremeshova' AND id != NEW.sender_id LOOP
    INSERT INTO notifications (user_id, type, payload, read) VALUES (r.id, 'admin_chat_message', jsonb_build_object('conversation_id', NEW.conversation_id, 'from_user_id', NEW.sender_id, 'message_id', NEW.id), false);
  END LOOP;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS tr_notify_admins_on_admin_chat ON messages;
CREATE TRIGGER tr_notify_admins_on_admin_chat AFTER INSERT ON messages FOR EACH ROW EXECUTE FUNCTION notify_admins_on_admin_chat_message();

-- 7. create_dm_conversation (для DM с друзьями)
...
-- 9. company_stage в profiles (для admin_update_profile)
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS company_stage TEXT;
-- 10. Внутрисайтовая валюта
