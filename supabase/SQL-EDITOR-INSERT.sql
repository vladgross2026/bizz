-- =============================================================================
-- BIZFORUM — ВЕСЬ SQL В ОДНОМ ФАЙЛЕ
-- Скопируйте в Supabase: SQL Editor → New query → вставьте → Run
...
AS $$
BEGIN
  IF NEW.verified IS DISTINCT FROM OLD.verified AND auth.uid() = OLD.id AND NOT is_admin() THEN NEW.verified := OLD.verified; END IF;
  RETURN NEW;
END;
$$;
...
AS $$
BEGIN
  RETURN QUERY SELECT pr.id, pr.first_name, pr.last_name, pr.company FROM profiles pr
  WHERE pr.id != auth.uid() AND pr.verified = true
    AND (p_query IS NULL OR trim(p_query) = '' OR pr.first_name ILIKE '%' || trim(p_query) || '%' OR pr.last_name ILIKE '%' || trim(p_query) || '%')
  ORDER BY pr.first_name, pr.last_name LIMIT 30;
END;
$$;
...
-- ========== ЧАСТЬ 2: УВЕДОМЛЕНИЯ, ЧАТЫ, ПОДПИСКИ (ITOGOVYJ + CONSOLIDATED) ==========
...
-- ========== ЧАСТЬ 3: РЕАКЦИИ НА КОММЕНТАРИЯХ, ЛУЧШИЙ ОТВЕТ, БЕЙДЖИ (apply-all) ==========
...
ALTER TABLE comment_reactions ADD CONSTRAINT comment_reactions_type_check CHECK (type IN ('muzhik','koroleva','rzhaka','fire','fu','grustno','babki','hahaha','useful'));
...
AS $$
DECLARE helpful_cnt BIGINT; stage TEXT;
BEGIN
  SELECT pr.company_stage INTO stage FROM profiles pr WHERE pr.id = p_user_id;
  SELECT COUNT(DISTINCT cr.comment_id) INTO helpful_cnt FROM comments c JOIN comment_reactions cr ON cr.comment_id = c.id AND cr.type = 'useful' WHERE c.author_id = p_user_id;
...
-- ========== ЧАСТЬ 4: ПРОЧИТАННЫЕ СООБЩЕНИЯ, UNREAD ==========
...
-- ========== ЧАСТЬ 5: admin_update_profile (полная версия), APPLY-ADDITIONS, RUN-IN-SUPABASE ==========
...
RETURNS TABLE(author_id UUID, cnt BIGINT) LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE AS $$
  SELECT s.author_id, count(*)::BIGINT FROM subscriptions s WHERE s.author_id = ANY(p_author_ids) GROUP BY s.author_id;
$$;
...
-- Подписка: subscription_ends_at, post_opens, get_subscription_status
...
BEGIN
  IF NOT is_admin() THEN RETURN; END IF;
  UPDATE profiles SET first_name = COALESCE(p_first_name, first_name), last_name = COALESCE(p_last_name, last_name), company = COALESCE(p_company, company), verified = COALESCE(p_verified, verified), company_stage = COALESCE(p_company_stage, company_stage), balance = CASE WHEN p_balance IS NOT NULL THEN p_balance ELSE balance END, subscription_ends_at = CASE WHEN p_subscription_ends_at IS NOT NULL THEN p_subscription_ends_at ELSE subscription_ends_at END, updated_at = now() WHERE id = p_id;
END;
$$;
